...
import (
        ...
        "log"
        "github.com/ravisanwal/goland/packa/inner"
)

var person = `CREATE TABLE IF NOT EXISTS person (first_name varchar(100), last_name varchar(100), email varchar(100));`
var place = `CREATE TABLE IF NOT EXISTS place (country text, city text NULL, telcode integer);`
...
func main() {
        ...
        db, err := sqlx.Open("mysql", "ravis:ravis@tcp(ravislx.local:3306)/test?autocommit=false")
        if err != nil {
                log.Fatalln(err)
        }
